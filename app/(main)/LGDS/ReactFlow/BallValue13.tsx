...
export default function BallValue13() {
        ...
        if (ws.current) {
            ws.current.onmessage = (event) => {
                ...
                if (dataReceived.data && dataReceived.data.data.length > 0) {
                    const ballValue =
                        dataReceived.data.data[0].latest.ATTRIBUTE.BallValue_13
                            .value;
                    setUpData(ballValue);
                    ...
                    const updatedData =
                        dataReceived.update[0].latest.ATTRIBUTE.BallValue_13
                            .value;
                    const updateTS =
                        dataReceived.update[0].latest.ATTRIBUTE.BallValue_13.ts;
...
PostTelemetry_ZOVC,                
        ...
        const fetchData = async () => {
            try {
                ...
                setData(res.data);
                const ballValue = res.data.find((item: any) => item.key === "BallValue_13")?.value;
            } catch (error) {
