...
export default function BallValue11() {
        ...
        if (ws.current) {
            ws.current.onmessage = (event) => {
                ...
                if (dataReceived.data && dataReceived.data.data.length > 0) {
                    const ballValue =
                        dataReceived.data.data[0].latest.ATTRIBUTE.BallValue_11
                            .value;
                    setUpData(ballValue);
                    ...
                    const updatedData =
                        dataReceived.update[0].latest.ATTRIBUTE.BallValue_11
                            .value;
                    const updateTS =
                        dataReceived.update[0].latest.ATTRIBUTE.BallValue_11.ts;
...
PostTelemetry_ZOVC,                
        ...
        const fetchData = async () => {
            try {
                ...
                setData(res.data);
                const ballValue = res.data.find((item: any) => item.key === "BallValue_11")?.value;
            } catch (error) {
