...
export default function BallVavleSDV_BOTTOM1() {
        ...
        if (ws.current) {
            ws.current.onmessage = (event) => {
                ...
                if (dataReceived.data && dataReceived.data.data.length > 0) {
                    const ballValue =
                        dataReceived.data.data[0].latest.ATTRIBUTE.BallVavleSDV_BOTTOM
                            .value;
                    setUpData(ballValue);
                    ...
                    const updatedData =
                        dataReceived.update[0].latest.ATTRIBUTE.BallVavleSDV_BOTTOM
                            .value;
                    const updateTS =
                        dataReceived.update[0].latest.ATTRIBUTE.BallVavleSDV_BOTTOM.ts;
    ...
    const handleButtonClick = async () => {
        ...
        const fetchData = async () => {
            try {
                ...
                setData(res.data);
                const ballValue = res.data.find((item: any) => item.key === "BallVavleSDV_BOTTOM")?.value;
