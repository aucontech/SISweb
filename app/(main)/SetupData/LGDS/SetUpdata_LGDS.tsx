...
// =================================================================================================================== 
 ...
 useEffect(() => {
    ...
    if (!isNaN(FC_02_Yesterday_Values_VolumeValue) && !isNaN(highValue) && !isNaN(lowValue) && !maintainFC_02_Yesterday_Values_Volume) {
     ...
     const ChangeMaintainFC_02_Yesterday_Values_Volume = async () => {
      ...
      const [audioPlayingPT_1003, setAudioPlayingPT_1003] = useState(false);
      const [inputValuePT_1003, setInputValuePT_1003] = useState<any>();
      const [inputValue2PT_1003, setInputValue2PT_1003] = useState<any>();
      ...
      const [maintainPT_1003, setMaintainPT_1003] = useState<boolean>(false);
          ...
          useEffect(() => {
              if (typeof PT_1003_High === 'string' && typeof PT_1003_Low === 'string' && PT_1003 !== null && maintainPT_1003 === false
              ) {
                  const highValue = parseFloat(PT_1003_High);
                  const lowValue = parseFloat(PT_1003_Low);
                  const PT_1003Value = parseFloat(PT_1003);
          
                  if (!isNaN(highValue) && !isNaN(lowValue) && !isNaN(PT_1003Value)) {
                      if (highValue <= PT_1003Value || PT_1003Value <= lowValue) {
                          if (!audioPlayingPT_1003) {
                              audioRef.current?.play();
                              setAudioPlayingPT_1003(true);
                              setExceedThresholdPT_1003(true);
                          }
                      } else {
                         setAudioPlayingPT_1003(false);
                         setExceedThresholdPT_1003(false);
                      }
                  } 
              } 
          }, [PT_1003_High, PT_1003, audioPlayingPT_1003, PT_1003_Low,maintainPT_1003]);
          ...
          const handleInputChangePT_1003 = (event: any) => {
              const newValue = event.target.value;
              setInputValuePT_1003(newValue);
          };
      
          const handleInputChange2PT_1003 = (event: any) => {
              const newValue2 = event.target.value;
              setInputValue2PT_1003(newValue2);
          };
          const ChangeMaintainPT_1003 = async () => {
              try {
                  const newValue = !maintainPT_1003;
                  await httpApi.post(
                      `/plugins/telemetry/DEVICE/${id_LGDS}/SERVER_SCOPE`,
                      { PT_1003_Maintain: newValue }
                  );
                  setMaintainPT_1003(newValue);
 ...
 // =================================================================================================================== 
        ...
        try {
            await httpApi.post(
...

                {
                    ...
                    PT_1003_High: inputValuePT_1003,PT_1003_Low:inputValue2PT_1003,
            ...
            setPT_1003_High(inputValuePT_1003);
            setPT_1003_Low(inputValue2PT_1003);
    ...
    useEffect(() => {
        ...
        setInputValuePT_1003(PT_1003_High); 
        setInputValue2PT_1003(PT_1003_Low); 
...
//=====================================================================================
  ...
  const mainCategoryFC = {
    FC01:  <span  style={{display:'flex',textAlign:'center', justifyContent:'space-between'  }}> FC-1001 -  Parameters & Configurations   <div style={{display:'flex' , textAlign:'center', alignItems:'center',}}>  
        <Checkbox disabled={TECH_OPER}
            style={{ marginRight: 5 }}
            onChange={handleCheckboxChangeFC}
            checked={checkMaintainingFC}
...
></Checkbox>
    ...
    const PLC = [
        ...
        value: <span style={combineCss.CSSPT_1003} > {formatValue(PT_1003)} {nameValue.Bara}</span> , 
         high: <InputText disabled={TECHNIAN_AUTH} style={combineCss.CSSPT_1003}   placeholder='High' step="0.1" type='number' value={inputValuePT_1003} onChange={handleInputChangePT_1003} inputMode="decimal" />, 
         low:  <InputText disabled={TECHNIAN_AUTH} style={combineCss.CSSPT_1003}   placeholder='Low' step="0.1" type='number' value={inputValue2PT_1003} onChange={handleInputChange2PT_1003} inputMode="decimal" />,
         update:  <Button className='buttonUpdateSetData' onClick={confirmUpData} label='Update' disabled={AuthUpdatePCV} />,
         Maintain:   <Checkbox disabled={TECH_OPER}
         style={{ marginRight: 20, }}
   ...
   ></Checkbox>
      ...
      value: <span style={combineCss.CSSFC_Conn_STT} > {formatValue(FC_Conn_STT)} {DataFC_Conn_STT}</span> , 
       high: <InputText disabled={TECHNIAN_AUTH} style={combineCss.CSSFC_Conn_STT}   placeholder='High' step="0.1" type='number' value={inputValueFC_Conn_STT} onChange={handleInputChangeFC_Conn_STT} inputMode="decimal" />, 
   ...
   ></Checkbox>
    ...
    const PT_1103_Parameter = [
        ...
        value: <span style={combineCss.CSSPT_1003} > {formatValue(PT_1003)} {nameValue.BARG}</span> , 
        high: <InputText    disabled={TECHNIAN_AUTH} style={combineCss.CSSPT_1003}   placeholder='High' step="0.1" type='number' value={inputValuePT_1003} onChange={handleInputChangePT_1003} inputMode="decimal" />, 
        low:  <InputText    disabled={TECHNIAN_AUTH} style={combineCss.CSSPT_1003}   placeholder='Low' step="0.1" type='number' value={inputValue2PT_1003} onChange={handleInputChange2PT_1003} inputMode="decimal" />,
        update:  <Button className='buttonUpdateSetData' onClick={confirmUpData} label='Update'  disabled={AuthUpdatePCV} />,
        Maintain:   <Checkbox disabled={TECH_OPER}
