...
import TelemetryIGUACU from '../IGUACU/Telemetry-IGUACU';
import TelemetryTHACHTHAT from '../THACHTHAT/Telemetry-THACHTHAT';
import TelemetryOTSUKA from '../OTSUKA/Telemetry-OTSUKA';
import { ProgressSpinner } from 'primereact/progressspinner';
import { id_IGUECU, id_OTSUKA, id_THACHTHAT } from '@/app/(main)/data-table-device/ID-DEVICE/IdDevice';

export default function ListTelemetry() {
    ...
    const handleChangeID = (value: any) => {
        const selectedDeviceObject = devices.find(device => device.id.id === value);
    ...
    const renderGraphicComponent = () => {
        const selectedDeviceObject = devices.find(device => device.id.id === selectedDevice);
        if (selectedDeviceObject) {
            switch (selectedDeviceObject.id.id) {
                case id_IGUECU:
                    return <TelemetryIGUACU />;
                case id_THACHTHAT:
                    return <TelemetryTHACHTHAT />;
                    case id_OTSUKA:
                     return <TelemetryOTSUKA />;
                default:
                    return null;
            }
        }
    ...
    return (
        <div>
            <Dropdown
                ...
                options={devices.map(device => ({ label: device.name, value: device.id.id }))} 
